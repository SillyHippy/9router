...
export default function UsageStats({ period: periodProp, setPeriod: setPeriodProp, hidePeriodSelector = false } = {}) {
  ...
  useEffect(() => {
    Promise.all([
      fetch((process.env.NEXT_PUBLIC_BASE_PATH || "") + "/api/providers").then((r) => r.ok ? r.json() : null),
      fetch((process.env.NEXT_PUBLIC_BASE_PATH || "") + "/api/provider-nodes").then((r) => r.ok ? r.json() : null),
    ])
      .then(([d, nodesData]) => {
